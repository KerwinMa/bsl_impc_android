...
public class RotateImageView extends TwoStateImageView implements Rotatable {
    //TODO 内存泄漏，此类一直持有Cameramain的引用而未被释放，addView而未被remove，移除时请确保移除了监听，请注意查找
    ...
    public interface IndicatorClickListener {
        void onIndicatorClick(View view, boolean showed);
        void onPopItemClick(View view);
    }
    
    protected IndicatorClickListener mClickListener;
    ...
    public void setOrientation(int degree, boolean animation) {
        ...
        degree = degree >= 0 ? degree % MathConstants.DEGREE_ROUND 
                : degree % MathConstants.DEGREE_ROUND + MathConstants.DEGREE_ROUND;
        ...
        if (mEnableAnimation) {
            ...
            diff = diff >= 0 ? diff : MathConstants.DEGREE_ROUND + diff; // make it in range [0, 359]

            // Make it in range [-179, 180]. That's the shorted distance between
            // the
            // two angles
            diff = diff > MathConstants.DEGREE_HALF_ROUND ? diff - MathConstants.DEGREE_ROUND : diff;
    ...
    public void setIndicatorChangeListener(IndicatorClickListener listener) {
